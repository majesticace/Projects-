...
AWS_S3_BUCKET = "tempsens"  # edit this
AWS_S3_KEY_PREFIX = "tmp"  # edit this NOTE: refers to s3 object key
                            # not access key
...
while True:
    ...
    # Convert 'temp' column to numeric data type
    df['temp'] = pd.to_numeric(df['temp'], errors='coerce')
    df['humidity'] = pd.to_numeric(df['humidity'], errors='coerce')  # Convert 'humidity' column to numeric

    current_temp = df['temp'].tail(1).iloc[0]  # Extract single value from Series
    current_humidity = df['humidity'].tail(1).iloc[0]  # Extract single value from Series
    ...
    with placeholder.container():
        ...
        metric3.metric(
            ...
            value=round(df['temp'].mean(), 2)  # Now safe to calculate mean
        )
        ...
        gauge_chart = go.Figure(go.Indicator(
            ...
            value=round(float(current_humidity)),  # Convert to float before rounding
            domain={'x': [0, 1], 'y': [0, 1]},
            delta={'reference': 50},
            title={'text': 'Humidity'},
